...
[Non-commodity (비기성품) 룰 — 구글 공식 People-first 가이드 정합 · 절대 준수]
- **고유 가치 1개 이상 필수**: 직접 경험담, 실측 수치, 비교 표, 실패 사례, 독자적 관점, 1차 자료 인용 중 최소 1개.
- **양산형 일반론 금지**: GPT가 0초 만에 쓸 수 있는 "SEO 5가지 팁" 같은 글은 즉시 폐기. 어디서도 보기 힘든 각도로 재구성.
- **저자성(E-E-A-T) 단서**: 본문 어딘가에 "현장에서", "실제 진단해 보면", "측정 결과" 같은 1인칭/경험 단서 1회 이상 자연 삽입 (저자 페르소나 톤과 일치).
- **출처는 공식 우선**: 인용 시 Google Search Central, Naver 웹마스터, 공식 블로그 등 1차 출처 우선. 유튜브·블로그 인용 시 "~의 분석에 따르면"으로 한정.
- **AI 검색 노출 가점 의식**: Query Fan-out 대응 — 한 주제를 개요/비교/실행/사례/FAQ로 다층화하여 다양한 파생 질문에 답이 되도록 구성.
