...
function getCardClasses(score: number) {
  ...
  switch (level) {
    case "critical":
      return "ring-2 ring-score-poor/40 bg-gradient-to-b from-score-poor/[0.04] to-card shadow-[0_0_24px_-4px_hsl(var(--score-poor)/0.18)]";
    case "warning":
...
function getHeaderBadge(score: number) {
  ...
  if (level === "critical") {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-score-poor/12 text-score-poor border border-score-poor/20 animate-pulse">
...
function SignalBar({ name, score, weight }: { name: string; score: number; weight: number }) {
  ...
  return (
    <div className={`flex items-center gap-2.5 py-0.5 ${isCritical ? "px-2 -mx-2 rounded-md bg-score-poor/[0.06]" : ""}`}>
...
function ImprovementRow({ icon, label, item, urgent }: { icon: React.ReactNode; label: string; item: Improvement; urgent?: boolean }) {
  return (
    <div className={`flex items-start gap-2 ${urgent ? "px-3 py-2 -mx-2 rounded-lg bg-score-poor/[0.05] border border-score-poor/10" : ""}`}>
      {icon}
      ...
      <span className={`shrink-0 px-2 py-0.5 rounded text-xs font-semibold border ${
        urgent
  ...
  return (
    ...
    >
      ...
      <div className="pt-6 px-6 pb-4 flex flex-col items-center">
        ...
        <span className={`text-sm font-semibold ${gradeColor} -mt-1`}>{grade}</span>
      ...
      {axis.scoreCap && (
        <div className={`mx-5 mb-3 flex items-start gap-2 px-3.5 py-2.5 rounded-lg border ${
          ...
          <span className={`text-[13px] leading-relaxed ${isCritical ? "text-score-poor font-semibold" : "text-score-warning"}`}>
            {axis.scoreCap}
          </span>
        </div>
      )}
