...
export default function StickyBottomCTA() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-sm border-t border-border">
      <div className="container max-w-4xl mx-auto flex items-center gap-3 px-3 py-2 sm:px-4 sm:py-3">
        ...
        <div className="flex-1 flex gap-1.5 sm:gap-2 justify-end">
          <input
            ...
            className="flex-1 sm:max-w-[240px] min-w-0 h-8 sm:h-9 px-3 rounded-lg border border-input bg-muted/30 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-xs sm:text-sm transition-all"
          />
          <button
            onClick={handleSubmit}
            disabled={status === "loading"}
            className="shrink-0 h-8 sm:h-9 px-3.5 sm:px-4 rounded-lg gradient-primary text-primary-foreground font-semibold text-xs sm:text-sm hover:opacity-90 transition-opacity whitespace-nowrap disabled:opacity-50"
          >
