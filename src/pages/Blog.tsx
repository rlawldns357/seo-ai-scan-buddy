...
function getBrandThumbnail(slug: string, category: string) {
  // ChatGPT icon (hexagonal knot)
  const ChatGPTIcon = ({ size = 32 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <path d="M22.282 9.821a5.985 5.985 0 0 0-.516-4.91 6.046 6.046 0 0 0-6.51-2.9A6.065 6.065 0 0 0 4.981 4.18a5.985 5.985 0 0 0-3.998 2.9 6.046 6.046 0 0 0 .743 7.097 5.98 5.98 0 0 0 .51 4.911 6.051 6.051 0 0 0 6.515 2.9A5.985 5.985 0 0 0 13.26 24a6.056 6.056 0 0 0 5.772-4.206 5.99 5.99 0 0 0 3.997-2.9 6.056 6.056 0 0 0-.747-7.073zM13.26 22.43a4.476 4.476 0 0 1-2.876-1.04l.141-.081 4.779-2.758a.795.795 0 0 0 .392-.681v-6.737l2.02 1.168a.071.071 0 0 1 .038.052v5.583a4.504 4.504 0 0 1-4.494 4.494zM3.6 18.304a4.47 4.47 0 0 1-.535-3.014l.142.085 4.783 2.759a.771.771 0 0 0 .78 0l5.843-3.369v2.332a.08.08 0 0 1-.033.062L9.74 19.95a4.5 4.5 0 0 1-6.14-1.646zM2.34 7.896a4.485 4.485 0 0 1 2.366-1.973V11.6a.766.766 0 0 0 .388.676l5.815 3.355-2.02 1.168a.076.076 0 0 1-.071 0l-4.83-2.786A4.504 4.504 0 0 1 2.34 7.872zm16.597 3.855l-5.833-3.387L15.119 7.2a.076.076 0 0 1 .071 0l4.83 2.791a4.494 4.494 0 0 1-.676 8.105v-5.678a.79.79 0 0 0-.407-.667zm2.01-3.023l-.141-.085-4.774-2.782a.776.776 0 0 0-.785 0L9.409 9.23V6.897a.066.066 0 0 1 .028-.061l4.83-2.787a4.5 4.5 0 0 1 6.68 4.66zm-12.64 4.135l-2.02-1.164a.08.08 0 0 1-.038-.057V6.075a4.5 4.5 0 0 1 7.375-3.453l-.142.08L8.704 5.46a.795.795 0 0 0-.393.681zm1.097-2.365l2.602-1.5 2.607 1.5v2.999l-2.597 1.5-2.607-1.5z" fill="#1A1A1A"/>
    </svg>
  );
  ...
  const ClaudeIcon = ({ size = 28 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((angle) => (
        <line key={angle} x1="12" y1="12" x2={12 + 10 * Math.cos((angle * Math.PI) / 180)} y2={12 + 10 * Math.sin((angle * Math.PI) / 180)} stroke="#DA7756" strokeWidth="1.8" strokeLinecap="round" />
      ))}
    </svg>
  );

  // Perplexity geometric icon
  const PerplexityIcon = ({ size = 28 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <path d="M12 2L14 8H20L15 12L17 18L12 14L7 18L9 12L4 8H10L12 2Z" stroke="#1DBAB0" strokeWidth="1.5" fill="none"/>
      <path d="M12 6L12 18M6 10L18 10" stroke="#1DBAB0" strokeWidth="1.2" strokeLinecap="round"/>
    </svg>
  );

  // Gemini 4-pointed star
  const GeminiIcon = ({ size = 28 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <defs>
      ...
      <path d="M12 2C12 2 14 8 16 10C18 12 24 12 24 12C24 12 18 12 16 14C14 16 12 22 12 22C12 22 10 16 8 14C6 12 0 12 0 12C0 12 6 12 8 10C10 8 12 2 12 2Z" fill="url(#geminiGrad)"/>
    </svg>
  );
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-center gap-1">
            <ChatGPTIcon size={28} />
            <span className="text-sm font-semibold" style={{ fontFamily: "'Inter', sans-serif", color: "#1A1A1A" }}>ChatGPT</span>
          </div>
          <span className="text-xs font-bold text-muted-foreground/50">VS</span>
          <div className="flex flex-col items-center gap-1">
            <GeminiIcon size={28} />
            <span className="text-sm font-semibold" style={{ fontFamily: "'Inter', sans-serif", color: "#4285F4" }}>Gemini</span>
          </div>
        </div>
      </div>
    );
  ...
  if (isChatGPTPost(slug)) {
    return (
      <div className="flex flex-col items-center gap-2">
        <ChatGPTIcon size={36} />
        <span className="text-2xl font-semibold tracking-tight" style={{ fontFamily: "'Inter', sans-serif", color: "#1A1A1A" }}>ChatGPT</span>
      </div>
    );
  }
  // Perplexity
  if (isPerplexityPost(slug)) {
    return (
      <div className="flex items-center gap-2">
        <PerplexityIcon size={30} />
        <span className="text-2xl tracking-tight" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500, color: "#1B1B1B", letterSpacing: "-0.03em" }}>perplexity</span>
      </div>
    );
  }
  // Claude
  if (isClaudePost(slug)) {
    return (
      <div className="flex items-center gap-2.5">
        <ClaudeIcon size={30} />
        <span className="text-[28px]" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontWeight: 400, color: "#1A1A1A", letterSpacing: "-0.01em" }}>Claude</span>
      </div>
    );
  }
  // 뤼튼 / Wrtn
  if (isWrtnPost(slug)) {
    return (
      <div className="flex items-center justify-center w-16 h-16 rounded-xl" style={{ backgroundColor: "#FF6B00" }}>
        <span className="text-2xl font-black tracking-tight" style={{ fontFamily: "'Inter', sans-serif", color: "#FFFFFF", letterSpacing: "-0.02em" }}>wrtn.</span>
      </div>
    );
  }
  // Google AI Overview
