...
**점수대별 마이크로카피 (배지/카드 옆 한 줄, 형식: `{점수}점 · {권고}`)**
| 점수 구간 | 라벨 (준비/성장 공용) | 색상 |
|---|---|---|
| 90~100 | `검수 불필요 · 바로 발행 OK` | blue |
| 75~89 | `검수 후 발행 추천` (예: **78점 · 검수 후 발행 추천**) | blue |
| 50~74 | `보강 후 발행 권장` | orange |
| 0~49 | `재작성 권장` | red |

- 큐 배지 기본 표기: `{점수}점 · {라벨}` 한 줄.
- 카드/모달에서는 라벨 아래 가장 약한 축 사유를 한 줄 더 노출 (예: "AEO가 가장 낮아요 — 직접 답변 부재").
- 발행 후 컨텍스트에서는 Δ를 라벨 앞에 붙임: `78점 (+6) · 안정적 성장 중`.
- "추천/권장"은 강제가 아님을 의미. 사용자는 어느 점수든 발행 가능.
